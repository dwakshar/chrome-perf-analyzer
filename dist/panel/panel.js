...
const runtimeCanvas = $("runtime-canvas");
const healthCanvas = $("health-canvas");
...
const vitalsCaption = $("vitals-caption");
const runtimeCaption = $("runtime-caption");
const healthCaption = $("health-caption");
...
function updateMetricEl(element, value, unit, thresholds, decimals = 0) {
    if (value === null) {
        element.textContent = "-";
...
function renderRuntimeMetrics(snapshot) {
    ...
    runtimeEls.longTasks.className = `metric-value ${longTaskCount === 0
        ? "good"
        : longTaskCount < 5
            ? "needs-improvement"
            : "poor"}`;
...
function renderChartsAndSummary(snapshot) {
    syncAllCanvasResolutions();
    renderTimeline();
...
function renderIssues(issues) {
    ...
    const sorted = [...issues].sort((left, right) => severityOrder[left.severity] - severityOrder[right.severity]);
...
function renderTimeline() {
    ...
    if (timelineHistory.length === 0) {
        drawCenteredEmptyState(timelineCtx, width, height, "Waiting for observed vitals from the page");
        return;
    }
    ...
    for (const point of timelineHistory) {
        ...
        if (inp !== null)
            maxMs = Math.max(maxMs, inp);
        if (cls !== null)
            maxCls = Math.max(maxCls, cls);
    }
    const xForTimestamp = (timestamp) => paddingLeft + ((timestamp - firstPoint.timestamp) / span) * plotWidth;
    const yForMs = (value) => paddingTop + plotHeight - (value / maxMs) * plotHeight * 0.76;
    const yForCls = (value) => paddingTop + plotHeight - (value / maxCls) * plotHeight * 0.24;
    drawThresholdBand(timelineCtx, paddingLeft, paddingTop, plotWidth, plotHeight, 0, 2500, maxMs, "rgba(61, 220, 132, 0.08)");
    drawThresholdBand(timelineCtx, paddingLeft, paddingTop, plotWidth, plotHeight, 2500, 4000, maxMs, "rgba(245, 166, 35, 0.07)");
    drawThresholdBand(timelineCtx, paddingLeft, paddingTop, plotWidth, plotHeight, 4000, maxMs, maxMs, "rgba(255, 95, 95, 0.06)");
    drawThresholdBand(timelineCtx, paddingLeft, paddingTop + plotHeight * 0.76, plotWidth, plotHeight * 0.24, 0, 0.1, maxCls, "rgba(61, 220, 132, 0.06)");
    drawThresholdBand(timelineCtx, paddingLeft, paddingTop + plotHeight * 0.76, plotWidth, plotHeight * 0.24, 0.1, 0.25, maxCls, "rgba(245, 166, 35, 0.05)");
    drawThresholdBand(timelineCtx, paddingLeft, paddingTop + plotHeight * 0.76, plotWidth, plotHeight * 0.24, 0.25, maxCls, maxCls, "rgba(255, 95, 95, 0.04)");
    ...
    drawTimeMarkers(timelineCtx, paddingLeft, paddingTop + plotHeight + 14, plotWidth, firstPoint.timestamp, lastPoint.timestamp);
...
function renderRuntimeChart(snapshot) {
    if (!runtimeCtx)
        return;
    ...
    if (longTasks.length === 0 && heapSamples.length === 0) {
        drawCenteredEmptyState(runtimeCtx, width, height, "Long tasks and heap samples will appear here");
        return;
    }
    const plotWidth = width - paddingLeft - paddingRight;
    const plotHeight = height - paddingTop - paddingBottom;
    if (longTasks.length > 0) {
        ...
        longTasks.forEach((task, index) => {
            ...
            runtimeCtx.fillRect(x, paddingTop + plotHeight - barHeight, Math.max(3, barWidth - 3), barHeight);
        });
...
function renderHealthBars(cwv) {
    ...
    metrics.forEach((metric, index) => {
        ...
        const scale = (value) => startX + (Math.min(value, metric.max) / metric.max) * barWidth;
...
function renderRadarChart(cwv) {
    ...
    if (axes.some((axis) => axis.score !== null)) {
        radarCtx.beginPath();
        axes.forEach((axis, index) => {
            const point = polarPoint(centerX, centerY, radius * (axis.score ?? 0.12), index, axes.length);
            if (index === 0)
                radarCtx.moveTo(point.x, point.y);
            else
                radarCtx.lineTo(point.x, point.y);
...
function drawChartFrame(ctx, width, height, paddingLeft, paddingTop, paddingRight, paddingBottom) {
    const plotWidth = width - paddingLeft - paddingRight;
    const plotHeight = height - paddingTop - paddingBottom;
    ctx.save();
...
function drawThresholdBand(ctx, left, top, width, height, from, to, domainMax, color) {
    if (domainMax <= 0 || to <= from)
        return;
...
function drawTimeMarkers(ctx, left, y, width, firstTimestamp, lastTimestamp) {
...
function drawCenteredEmptyState(ctx, width, height, label) {
    ctx.save();
...
function drawLine(ctx, color, getValue, yForValue, xForTimestamp) {
    ctx.save();
...
function syncAllCanvasResolutions() {
...
function scoreMetric(value, thresholds, max) {
    if (value === null)
        return null;
...
function hasAnyMetric(cwv) {
    return [cwv.lcp, cwv.cls, cwv.inp, cwv.fcp, cwv.ttfb].some((value) => value !== null);
}
