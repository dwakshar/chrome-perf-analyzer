...
function wireLifecycle(panel: chrome.devtools.panels.ExtensionPanel): void {
  ...
  panel.onShown.addListener((panelWindow: Window) => {
    ...
    try {
      ...
      (panelWindow as PanelWindow).__PERF_TAB_ID__ = INSPECTED_TAB_ID;
    } catch (err) {
      ...
      console.warn(
        "[devtools] Failed to inject tab id into panel window:",
        err
      );
      return;
    }
...
/** Extended Window type for the panel page. */
interface PanelWindow extends Window {
  __PERF_TAB_ID__: number;
}
