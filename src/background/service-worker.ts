...
async function handleMessage(
  ...
  const effectiveTabId =
    (msg.tabId && msg.tabId !== -1 ? msg.tabId : undefined) ??
    sender?.tab?.id;
  ...
  (msg as ExtensionMessage).tabId = effectiveTabId;

  console.log(`[sw] Received: ${msg.type} for tab ${effectiveTabId}`);

  switch (msg.type) {
    case "DEVTOOLS_OPENED":
      return onDevToolsOpened(msg as ExtensionMessage<SessionInitPayload>);
    ...
    case "METRICS_UPDATE":
      return onMetricsUpdate(effectiveTabId, msg.payload as ContentScriptMetricsPayload);
...
async function onDevToolsOpened(
  msg: ExtensionMessage<SessionInitPayload>
): Promise<{ ok: boolean }> {
...
async function onPanelReady(
  ...
  } satisfies SessionHydratePayload;
}
...
function deriveIssuesFromCWV(cwv: CoreWebVitals): PerformanceIssue[] {
  ...
  if (cwv.lcp !== null && cwv.lcp > 2500) {
    issues.push({
      ...
      detail:
        "Largest Contentful Paint is slower than recommended. This often indicates heavy hero images or render-blocking resources.",
      recommendation:
        "Optimize above-the-fold images (compression, proper sizing, lazy-load offscreen) and reduce render-blocking CSS/JS.",
    });
  }
  ...
  if (cwv.cls !== null && cwv.cls > 0.1) {
    issues.push({
      ...
      detail:
        "Cumulative Layout Shift is above the recommended threshold, which means elements are moving around visibly during load.",
      recommendation:
        "Always reserve space for images/ads, avoid inserting content above existing content, and use transform-based animations.",
    });
  }
  ...
  if (cwv.fcp !== null && cwv.fcp > 1800) {
    issues.push({
      ...
      detail:
        "First Contentful Paint is slower than recommended, indicating that the initial render is delayed.",
      recommendation:
        "Reduce critical JavaScript and CSS on the initial path and consider server-side rendering or static generation where possible.",
    });
  }
  ...
  if (cwv.ttfb !== null && cwv.ttfb > 800) {
    issues.push({
      ...
      detail:
        "Time To First Byte is slow, which usually points to backend latency or slow CDN/hosting.",
      recommendation:
        "Profile server response times, enable caching (CDN, edge), and optimize database or API calls on the critical path.",
    });
  }
...
function computeCwvScore(cwv: CoreWebVitals): number {
  ...
  const penalize = (condition: boolean, mild: number, strong: number) => {
    if (!condition) return;
    score -= strong ?? mild;
  };
...
function routeCDPEvent(tabId: number, method: string, params?: unknown): void {
